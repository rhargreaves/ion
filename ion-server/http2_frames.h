...
struct Http2Setting {
    ...
    static Http2Setting parse(std::span<const uint8_t, 6> data) {
        return Http2Setting{
    ...
    void serialize(std::span<uint8_t, 6> data) const {
...
struct Http2GoAwayPayload {
    ...
    static Http2GoAwayPayload parse(std::span<const uint8_t, 8> data) {
    ...
    void serialize(std::span<uint8_t, 8> data) const {
...
struct Http2WindowUpdate {
    ...
    static Http2WindowUpdate parse(std::span<const uint8_t, 4> data) {
        return Http2WindowUpdate{
            .window_size_increment = load_uint32_be(data) & 0x7FFFFFFF,
        };
    }

    void serialize(std::span<uint8_t, 4> data) const {
